        ...
        <div>
          ...
          {chapters.map(chapter => (
            <label key={chapter._id} className="block">
              <input type="checkbox" checked={formData.selectedChapters.includes(chapter._id)} onChange={() => handleCheckboxChange(chapter._id)} className="mr-2" />
              {chapter.title}
            </label>
          ))}
        </div>

        <input type="number" name="numQuestions" value={formData.numQuestions} onChange={handleChange} placeholder="Number of Questions" className="w-full px-3 py-2 border rounded-lg" required />

        <input type="number" name="totalMarks" value={formData.totalMarks} onChange={handleChange} placeholder="Total Marks" className="w-full px-3 py-2 border rounded-lg" required />
