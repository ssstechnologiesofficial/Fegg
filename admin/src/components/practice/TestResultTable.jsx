...
const TestResultsTable = () => {
  ...
  const deleteResult = async (id) => {
    ...
    try {
      await fetch(SummaryApi.TestResult.url.replace(":id", id), { method: "DELETE" });
      fetchResults();
    } catch (error) {
  ...
  const handleEdit = (result) => {
    ...
    setFormData({ learnerName: result.learnerName, learnerId: result.learnerId, score: result.score });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white shadow-md rounded-lg">
      <h2 className="text-2xl font-bold text-center mb-4 text-red-500">Test Results</h2>
      ...
      {editingResult && (
        <div className="mb-6 p-4 bg-gray-100 border border-red-500 rounded">
          ...
          />
          <button onClick={updateResult} className="bg-green-500 text-white px-4 py-2 rounded">Update</button>
        </div>
      )}
        ...
        <tbody>
          {currentResults.map((result, index) => (
            <tr key={result._id} className="text-center">
              ...
              <td className={`border p-2 ${result.score >= 50 ? "text-green-600" : "text-blue-600"}`}>{result.score}</td>
              <td className="border p-2">{result.correctAnswers}</td>
              <td className="border p-2">{result.wrongAnswers}</td>
              <td className="border p-2">
                <button onClick={() => handleEdit(result)} className="bg-blue-500 text-white px-3 py-1 rounded mr-2">Edit</button>
                <button onClick={() => deleteResult(result._id)} className="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className="flex justify-between mt-4">
        ...
        <button
          onClick={() => setCurrentPage((prev) => (indexOfLastResult < results.length ? prev + 1 : prev))}
